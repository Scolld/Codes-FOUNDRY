...
async function generateNPC() {
  ...
  const tech = allItems.filter(i => i.type === "technology");

  const gear = [];

  if (weapons.length) gear.push(randomFrom(weapons).toObject());
  if (armors.length) gear.push(randomFrom(armors).toObject());
  ...
  if (tech.length) gear.push(randomFrom(tech).toObject());
